...
def upsert_rows_sqlite(conn, rows: Iterable[Dict[str, Any]]) -> int:
    ...
    for r in rows:
        conn.execute(
            ...
            (r.get("id"), r.get("title"), r.get("company"), r.get("url"), r.get("location"),
             int(bool(r.get("remote"))) if r.get("remote") is not None else None,
             r.get("created_at"), r.get("provider"), json.dumps(r.get("extra")) if r.get("extra") is not None else None,
