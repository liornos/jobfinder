...
def _first_path_segment(u: str, host: str) -> str | None:
    ...
    p = urlparse(u)
    if p.netloc.lower() != host.lower(): return None
...
async def _serpapi_search(query: str, api_key: str, num: int = 10) -> List[Dict]:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        data = r.json() or {}; return data.get("organic_results", [])
...
def _extract_orgs(results: List[Dict]) -> Tuple[List[str], List[str]]:
    ...
    for item in results:
        ...
        if org: gh.add(org); continue
        org = _first_path_segment(link, "jobs.lever.co")
        if org: lv.add(org); continue
    return sorted(gh), sorted(lv)

async def discover_companies(cities: List[str], keywords: List[str], sources: Iterable[str], limit: int=50, api_key: str | None=None) -> DiscoveryResult:
    if not api_key:
        api_key = os.getenv("SERPAPI_API_KEY")
    if not api_key:
        raise RuntimeError("SERPAPI_API_KEY missing. Set in environment or .env")
    wanted = {s.strip().lower() for s in (sources or []) if s} or {"greenhouse", "lever"}
    queries: List[str] = []; kws = " ".join(keywords) if keywords else ""
    for city in (cities or [""]):
        if "greenhouse" in wanted: queries.append(f'site:boards.greenhouse.io "{city}" {kws}'.strip())
        if "lever" in wanted: queries.append(f'site:jobs.lever.co "{city}" {kws}'.strip())
    results: List[Dict] = []; per_q = max(10, min(50, limit))
    for q in queries: results.extend(await _serpapi_search(q, api_key, num=per_q))
    ...
    companies = [c for _, c in sorted(uniq.items(), key=lambda kv: (kv[0][0], kv[0][1]))][:limit]
