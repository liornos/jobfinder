...
def test_discover_refresh_and_jobs(monkeypatch, serpapi_env, serpapi_stub, provider_stub, temp_db_url):
    serpapi_stub(
        {
    ...
    refresh_resp = client.post("/refresh", json={"companies": companies, "cities": ["Tel Aviv"], "keywords": ["engineer"]})
    assert refresh_resp.status_code == 200
    summary = refresh_resp.get_json()["summary"]
    assert summary["jobs_written"] == 2

    jobs_resp = client.get("/jobs", query_string={"cities": "Tel Aviv", "keywords": "engineer"})
    ...
    jobs_resp2 = client.get("/jobs", query_string={"cities": "Tel Aviv,Herzliya", "keywords": "engineer"})
